...
# Replace all of `when (currentTab) { ... }` up to the `if (showNoBandDialog)`
# using a regex that captures everything
regex = re.compile(r"[\t ]*when \(currentTab\) \{[\s\S]*?\}[\s\S]*?if \(showNoBandDialog\) \{")

# Since that might be tricky, let's just replace everything from `when (currentTab)` up to `if (showNoBandDialog)`
start_str = "when (currentTab) {"
start_idx = content.find(start_str)

if start_idx != -1:
    end_str = "if (showNoBandDialog) {"
    end_idx = content.find(end_str)
    
    # We want to replace from start_idx to end_idx with our block
    # We must backtrack to the start of the line for start_idx
    ...
    content = content[:line_start] + new_code + content[end_idx:]
    
    with open('./app/src/main/java/com/example/ui/MainHotspotScreen.kt', 'w') as f:
        f.write(content)
else:
    print("Could not find when (currentTab)")
